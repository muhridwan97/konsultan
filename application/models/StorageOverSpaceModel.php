...
/**
 * Class StorageOverSpaceModel
 */
class StorageOverSpaceModel extends MY_Model
{
    protected $table = 'storage_over_spaces';
    ...
    protected function getBaseQuery($branchId = null)
    {
        return parent::getBaseQuery($branchId)
            ->select([
                'ref_branches.branch',
                'COUNT(storage_over_space_customers.id) AS total_customer_over_space',
                'SUM(IF(storage_over_space_customers.status != "PENDING", 1, 0)) AS total_proceed',
                'SUM(IF(storage_over_space_customers.status = "PENDING", 1, 0)) AS total_pending',
                'SUM(IF(storage_over_space_customers.status = "VALIDATED", 1, 0)) AS total_validated',
                'SUM(IF(storage_over_space_customers.status = "SKIPPED", 1, 0)) AS total_skipped',
            ])
            ->join('storage_over_space_customers', 'storage_over_space_customers.id_storage_over_space = storage_over_spaces.id', 'left')
            ->join('ref_branches', 'ref_branches.id = storage_over_spaces.id_branch', 'left')
            ->group_by('storage_over_spaces.id');
    }
    ...
    public function getAll($filters = [], $withTrashed = false)
    {
        ...
        if (key_exists('type', $filters) && !empty($filters['type'])) {
            $baseQuery->where('storage_over_spaces.type', $filters['type']);
        }

        if (!empty($search)) {
            $baseQuery
                ->group_start()
                ->like('storage_over_spaces.date_from', $search)
                ->like('storage_over_spaces.date_to', $search)
                ->or_like('storage_over_spaces.description', $search)
                ->or_like('storage_over_spaces.status', $search)
                ->group_end();
        }
