...
class S3FileManager
{
    ...
    {
        if (preg_match('/\/$/', $sourceKeyname)) {
            return $this->moveObjects($sourceBucket, $sourceKeyname, $targetKeyname, $targetBucket);
        }
     ...
     */
    public function moveObjects($sourceBucket, $sourceKeyname, $targetKeyname, $targetBucket = null)
    {
        if (is_null($targetBucket)) {
            $targetBucket = $sourceBucket;
        }

        $sourceKeyname = rtrim($sourceKeyname, '/') . '/';
        $targetKeyname = rtrim($targetKeyname, '/') . '/';
        ...
        try {
            ...
            foreach ($results['Contents'] ?: [] as $file) {
                $targetKeynameDest = str_replace($sourceKeyname, $targetKeyname, $file['Key']);
                $copyResult = $this->copyObject($sourceBucket, $file['Key'], $targetKeynameDest, $targetBucket);
                if ($copyResult) {
                    $movedKeys[] = ['Key' => $file['Key']];
                }
            }
            return $this->deleteObjects($sourceBucket, $movedKeys);
        } catch (S3Exception $e) {
            log_message('error', $e->getMessage());
            return false;
        }
     ...
     * @return bool
     */
    public function deleteMatchingObjects($bucket, $matchKey)
    {
        try {
            $this->s3->deleteMatchingObjects($bucket, $matchKey);
            return true;
        } catch (S3Exception $e) {
            log_message('error', $e->getMessage());
            return false;
