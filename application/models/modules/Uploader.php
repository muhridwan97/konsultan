...
class Uploader extends CI_Model
{
    ...
    public function uploadToS3($input, $options = [])
    {
        ...
        $allowedFileType = get_if_exist($options, 'type', self::WHITELISTED_TYPE . '|' . strtoupper(self::WHITELISTED_TYPE));
    ...
    public function uploadTo($input, $options = [])
    {
        ...
        $fileType = get_if_exist($options, 'type', self::WHITELISTED_TYPE . '|' . strtoupper(self::WHITELISTED_TYPE));
    ...
    public function move($from, $to, $base = self::UPLOAD_PATH)
    {
        if ($this->driver == 's3') {
            if (preg_match('/\/$/', $from)) {
                return $this->s3FileManager->moveObjects(env('S3_BUCKET'), $from, $to);
            }
     ...
     * @return bool
     */
    public function uploadToWithRotate($input, $options = [],$rotate=270)
    {
        $basePath = get_if_exist($options, 'base_path', Uploader::UPLOAD_PATH);
        $uploadDir = get_if_exist($options, 'destination', 'temp');
        ...
        $maxWidth = get_if_exist($options, 'max_width', 5000);
        $maxHeight = get_if_exist($options, 'max_height', 5000);
        $fileName = get_if_exist($options, 'file_name', (!is_array($_FILES[$input]['name']) ? uniqid() . '.' . pathinfo($_FILES[$input]['name'], PATHINFO_EXTENSION) : ''));
            ...
            for ($i = 0; $i < $totalUploads; $i++) {
                if (empty($_FILES[$input]['name'][$i])) {
                    ...
                    if ($this->upload->do_upload($input . '_multiple')) {
                        ...
                        $config['source_image'] = $uploaded['full_path']; //get original image
                        $config['maintain_ratio'] = TRUE;
                        $config['rotation_angle'] = $rotate;//counter-clockwise angle of rotation
