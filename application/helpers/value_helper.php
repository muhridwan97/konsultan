...
if (!function_exists('numerical')) {
     ...
     * @return int|string
     */
    function numerical($number, $precision = 9, $trimmed = true, $dec_point = ',', $thousands_sep = '.')
    {
        if (empty($number)) {
            return 0;
        }
...
if (!function_exists('detect_chat_id')) {
    ...
    function detect_chat_id($chatId)
    {
        ...
        if (!(strpos($chatId, '@c.us') !== false) && (preg_match('/^08/', $chatId) || preg_match('/^628/', $chatId)) && strlen($chatId) <= 15) {
            $chatId = preg_replace('/^08/', '628', $chatId);
            $chatId .= '@c.us';
        } else if (!(strpos($chatId, '@g.us') !== false)) {
            $chatId .= '@g.us';
        }
...
if (!function_exists('cache_remember')) {
    /**
     * Get cache or remember if not exist.
     *
     * @param $key
     * @param $seconds
     * @param $callback
    ...
    function cache_remember($key, $seconds, $callback)
    {
        ...
        if (!$cachedData = $CI->cache->get($key)) {
            $cachedData = $callback();
            $CI->cache->save($key, $cachedData, $seconds);
        }
...
if (!function_exists('filter_data_by_keys')) {
    /**
     * Filter key by data.
     *
     * @param $data
     ...
     * @return array
     */
    function filter_data_by_keys($data, $keys)
    {
