<div class="invoice-wrapper-pdf">
    ...
    <table style="width: 100%" class="mb10 mt0">
        ...
        <tr>
            ...
            <td>:</td>
            <td colspan="4">
                Jl. Denpasar Blok 1 No.1 dan 16 KBN Marunda, Cilincing, Jakarta Utara 14120
            </td>
        </tr>
        <tr>
            ...
            <td width="15%"><strong>PPJK NAME</strong></td>
            <td width="2%">:</td>
            <td><?= $expedition ?></td>
        </tr>
        <tr>
            ...
            <td>
                <?php $inboundDates = []; ?>
                <?php $outboundDates = []; ?>
                <?php foreach ($invoiceDetails as $inboundDate): ?>
                    <?php if ($inboundDate['type'] == 'STORAGE') {
                        $dates = explode('-', $inboundDate['description']);
                        if(!empty($dates) && isset($dates[0])) {
                            $inboundDates[] = DateTime::createFromFormat("d/m/Y", trim($dates[0]))->format('d F Y');
                        }
                        if(!empty($dates) && isset($dates[1])) {
                            $outboundDates[] = DateTime::createFromFormat("d/m/Y", trim($dates[1]))->format('d F Y');
                        }
                    } ?>
                <?php endforeach ?>
                <?= implode(', ', array_unique($inboundDates)) //readable_date($invoice['inbound_date'], false) ?>
            </td>
        </tr>
        <tr>
            ...
            <td>:</td>
            <td><?= implode(', ', array_unique($outboundDates)); // readable_date($invoice['outbound_date'], false) ?></td>
        </tr>
        <tr>
            <td><strong>NO BC IN / OUT</strong></td>
            <td>:</td>
            <td colspan="4">
                <?= $invoice['no_reference_booking'] ?> /
                <?php foreach ($bookingOuts as $bookingOut): ?>
                    <?= implode(', ', array_column(if_empty($bookingOuts, []), 'no_reference')) ?>
                <?php endforeach ?>
            </td>
        </tr>
    </table>
    ...
    <table class="table-invoice" style="margin-bottom: 5px">
        ...
        <?php foreach ($invoiceDetails as $invoiceDetail): ?>
            ...
            <tr>
                <td>
                    <?php if($invoiceDetail['item_name'] == 'LIFT ON'): ?>
                        OUTBOUND (Lift On)
                    <?php elseif($invoiceDetail['item_name'] == 'OB TPS'): ?>
                        <?= $invoiceDetail['item_name'] ?> (Terlampir)
                    <?php else: ?>
                    ...
                    <?php endif; ?>
                    <?php if($invoiceDetail['item_name'] == 'INBOUND'): ?>
                        <br>
                        <small class="text-muted">
                            (Trucking, Port-PLB, Lift Off, Eseal, Doc BC 16)
                        </small>
                    <?php endif; ?>
                    <?php if($invoiceDetail['item_name'] == 'KARANTINA'): ?>
                        <br>
                        <small class="text-muted">
                            (Lift On, Lift Off, Ambil Sampel, Seal Botol)
                        </small>
                    <?php endif; ?>
                </td>
                <td>
    ...
    <table>
        <tr>
            <td>
                ...
                <p><?= if_empty(trim(preg_replace("/ \([^)]+\)/","", $invoice['item_summary'])), 'No item summary') ?></p>
                <?php $key = array_search('KARANTINA', array_column($invoiceDetails, 'item_name')); ?>
                <?php if($key): ?>
                <strong>Container Quarantine :</strong>
                <p><?= preg_replace("/ \([^)]+\)/","", $invoiceDetails[$key]['item_summary']) ?></p>
                <?php endif; ?>
            </td>
