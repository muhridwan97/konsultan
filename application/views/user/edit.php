<div class="box box-primary">
    ...
    <form action="<?= site_url('user/update/'.$user['id']) ?>" class="need-validation" role="form" method="post">
        <div class="box-body">
                            ...
                            <div class="col-sm-3">
                                <label class="radio">
                                    <input type="radio" class="form-control" id="user_type_internal" name="user_type" value="INTERNAL"
                                        <?= set_radio('user_type', 'INTERNAL', $user['user_type'] == 'INTERNAL') ?>> <span>INTERNAL</span>
                                </label>
                            </div>
                            <div class="col-sm-3">
                                <label class="radio">
                                    <input type="radio" class="form-control" id="user_type_external" name="user_type" value="EXTERNAL"
                                        <?= set_radio('user_type', 'EXTERNAL', $user['user_type'] == 'EXTERNAL') ?>> <span>EXTERNAL</span>
                                </label>
                            </div>
                        </div>
                    ...
                    <div class="form-group <?= form_error('status') == '' ?: 'has-error'; ?>">
                        ...
                        <select class="form-control select2" name="status" id="status" data-placeholder="Select status" required>
                            ...
                            <?php foreach ($statuses as $key => $value): ?>
                                <option value="<?= $key ?>" <?= set_select('status', $key, $user['status'] == $key) ?>>
                                    <?= $value ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                ...
                <div class="col-md-6">
                    <div class="form-group <?= form_error('password') == '' ?: 'has-error'; ?>">
                        ...
                        <input type="password" class="form-control" id="password" name="password"
                               placeholder="New password" minlength="5" maxlength="50">
                        <span class="help-block">Leave it blank if you don't intend to change the password</span>
                ...
                <div class="col-md-6">
                    <div class="form-group <?= form_error('confirm_password') == '' ?: 'has-error'; ?>">
                        ...
                        <input type="password" class="form-control" id="confirm_password" name="confirm_password"
                               placeholder="Confirm password" minlength="5" maxlength="50">
                                ...
                                <div class="row">
                                    <?php foreach ($roles as $role): ?>
                                        ...
                                        foreach ($userRoles as $userRole) {
                                            if ($role['id'] == $userRole['id'] && $branch['id'] == $userRole['id_branch']) {
                                                $hasRole = true;
                                                break;
                                            }
                                        }
                                            ...
                                            <div class="checkbox icheck" style="margin-top: 0">
                                                <label>
                                                    <input type="checkbox" name="roles[<?= $branch['id'] ?>][]" id="role_<?= $branch['id'] ?>_<?= $role['id'] ?>" value="<?= $role['id'] ?>"
                                                        <?php echo set_checkbox('roles', $role['id'], $hasRole); ?>>
                                                    &nbsp; <?= $role['role'] ?>
                                                </label>
                                            </div>
                                        </div>
        ...
        <div class="box-footer clearfix">
            ...
            <button type="submit" data-toggle="one-touch" class="btn btn-warning pull-right">Update User</button>
        </div>
    </form>
</div>