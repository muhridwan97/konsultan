$(function () {
    ...
    var forReload = tableJob.DataTable({
        ...
        serverSide: true,
        processing: true,
        ajax: baseUrl + 'work-order/data?' + queryString,
        order: [[0, "desc"]],
        columns: [
            {data: 'no'},
            {data: 'customer_name'},
        ...
        // , {
        //     targets: ['field-update-at'],
        //     render: function (data, type, full) {
        //         return `${$.trim(data) === '' ? '-' : moment(data).format('D MMMM YYYY HH:mm')}<br>
        //         <small class="text-muted">
        //             Updated By: ${$.trim(full.updater_name) === '' ? '-' : full.updater_name}
        //         </small>`;
        //     }
        // }
        , {
            targets: ['field-locked-at'],
            render: function (data, type, full) {
                return `${$.trim(data) === '' ? '-' : moment(data).format('D MMMM YYYY HH:mm')}<br>
                <small class="text-muted">
                    Locked By: ${$.trim(full.locker_name) === '' ? '-' : full.locker_name}
                </small>`;
            }
        }, {
            ...
            render: function (data, type, full) {
                ...
                } else if (full.status_validation === 'REJECTED') {
                    statusLabelValidated = 'danger';
                } else if (full.status_validation === 'FIXED') {
                    statusLabelValidated = 'warning';
                }
            ...
            render: function (data, type, full) {
                ...
                if(full.status === 'COMPLETED' && statusCompletedDate == true){
                    var unix_today = moment(moment(full.today).format('YYYY-MM-DD')).format('X'); //detik
                    var unix_completed = moment(moment(completedDate).format('YYYY-MM-DD')).format('X'); //detik
                    var unix_tomorrow_completed = moment(moment(completedDate).add(full.total_offday, 'days').format('YYYY-MM-DD')).format('X'); //detik
                ...
                if (full.gate_in_date>='2019-12-26 00:00:00' && (full.multiplier_goods=='-1'||full.multiplier_goods=='1')) {
                    if (full.status_validation === 'ON REVIEW') {
                        $(control).find('.action-reject').remove();
                        $(control).find('.action-fix').remove();

                        if (['PENDING', 'CHECKED', 'APPROVED', 'VALIDATED'].indexOf(full.status_validation) !== -1) {
                        ...
                        if (['PENDING', 'CHECKED', 'APPROVED', 'FIXED', 'VALIDATED'].indexOf(full.status_validation) !== -1) {
                            $(control).find('.action-fix').remove();
                        }

                        if (['PENDING' ,'CHECKED', 'REJECTED', 'VALIDATED'].indexOf(full.status_validation) !== -1) {
                            $(control).find('.action-reject').remove();
                            $(control).find('.action-validate').remove();
                        }
                    }
                    ...
                    if (full.status_validation === 'ON REVIEW') {
                        ...
                        if (['PENDING', 'FIXED', 'VALIDATED'].indexOf(full.status_validation) !== -1) {
                            $(control).find('.action-fix').remove();
                        }
                        if (['VALIDATED', 'REJECTED'].indexOf(full.status_validation) !== -1) {
                            $(control).find('.action-validate').remove();
                            $(control).find('.action-reject').remove();
                        }
                    }
